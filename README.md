...
Amortisation schedule, is a Restful API application, that creates and retrieves schedules for assets that are to be financed.
...
Postman or curl can be used to post and retrieve data from the API. To post to the application, loan details will need to be passed to the API.
The loan details passed to the API depends on whether the schedules are created with a balloon payment or not. If the parameters passed, 
do not contain a value for balloon payment, or the balloon payment is zero, then the payment will be calculated without the balloon payment. 
...
{"costOfAsset":25000, "depositPaid":5000,"yearlyInterest":7.5,"numberOfMonthlyPayments":12,"balloonPayment":1000}
...
```
Use your favourite REST client to trigger REST calls.
e.g: curl or postman.
